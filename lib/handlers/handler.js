...
class Handler {
  ...
  async sortEmployeesByLastName() {
    try {
      ...
      const table = Tables.sortEmployeesByLastNameTable();
  ...
  async sortEmployeesBySalary() {
    try {
      ...
      const table = Tables.sortEmployeesBySalaryTable();
      ...
      employees.forEach((employee, index) => {
        table.push([
          ...
          employee.last_name,
          employee.role_title,
          `${employee.role_salary}`, // Assuming role_salary is a number and formatted here
        ]);
      });
