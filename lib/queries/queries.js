...
const executeQuery = async (query, params) => {
  try {
    ...
    throw error;  // Re-throw to allow custom error handling by the caller
  }
};
...
const addDepartment = async (name, isTestDepartment = false) => {
  try {
    const existingDepartment = await executeQuery("SELECT * FROM department WHERE name = $1", [name]);

    if (existingDepartment.rows.length > 0) {
      return { success: false, message: "A department with the same name already exists." };
    }
    ...
    return { success: false, message: `Error adding department: ${error.message}` };
  }
};
...
const addRole = async (title, salary, departmentId) => {
  try {
    await executeQuery("INSERT INTO role (title, salary, department_id) VALUES ($1, $2, $3)", [title, salary, departmentId]);
...
const addEmployee = async (firstName, lastName, roleId, isManager, managerId) => {
  try {
    await executeQuery("INSERT INTO employee (first_name, last_name, role_id, is_manager, manager_id) VALUES ($1, $2, $3, $4, $5)",
      [firstName, lastName, roleId, isManager, managerId]);
...
const viewAllDepartments = async () => {
  try {
    ...
    // console.log("Query to fetch departments:", query);
    const result = await pool.query(query);
    // console.log("Result from database:", result);
    const departments = result.rows || []; // Ensure departments is always an array
...
const viewTotalBudgetByAllDepartments = async () => {
  try {
    ...
    departmentBudgetsResult.rows.forEach((row) => {
      totalBudget += parseInt(row.total_utilized_budget, 10); // Parse as integer to ensure numerical addition
    });

    // Format the total budget to include comma separators
    ...
    return { totalBudget: formattedTotalBudget, departments: result.rows };
  } catch (error) {
...
const viewTotalUtilizedBudgetByDepartment = async (departmentId) => {
  try {
    ...
    // Format the budget to ensure it always returns a string formatted as currency
    const budget = result.rows[0].total_utilized_budget;
...
const viewEmployeeDetailsByDepartment = async (departmentId) => {
  try {
    ...
    const employees = employeeResult.rows.map((employee) => {
      ...
      return {
        ...employee,
        role_salary: `$${employee.role_salary.toLocaleString()}`,
...
const viewAllEmployees = async () => {
 ...
 SELECT 
    ...
    e.first_name,
    e.last_name,
    r.title AS role_title,  -- Ensure this alias matches the JOIN clause
    d.name AS department_name,  -- Ensure this alias matches the JOIN clause
    '$' || r.salary AS salary,  -- Assuming the role includes salary information
    e.is_manager,
...
`);
    // console.log("Employees fetched:", result.rows); // Log to verify the data
    return result.rows;
  } catch (error) {
...
const viewAllManagers = async () => {
  try {
    ...
    `);
    // console.log("Fetched managers:", result.rows);
    return result.rows;
  } catch (error) {
...
const viewEmployeesByManager = async (managerId) => {
  if (!managerId) {
    ...
    return []; // or throw new Error("Invalid manager ID provided");
  }
...
const viewEmployeesByDepartment = async (departmentId) => {
  if (!departmentId) {
    ...
    return; // or throw new Error("Invalid department ID provided");
  }
  ...
  try {
    ...
    const employeeQuery = `
      SELECT
        ...
        e.first_name,
        e.last_name,
        r.id AS role_id, -- Correctly aliasing role_id
        r.title AS role_name, -- Correctly aliasing role_name
        '$' || r.salary AS salary,
        CONCAT(m.first_name, ' ', m.last_name) AS manager_name,
        d.name AS department_name
      FROM
    ...
    throw error; // Rethrowing the error might be harsh; consider handling it more gracefully
  }
};
  ...
  // try {
  //   const query = `
  //     SELECT e.id, e.first_name, e.last_name, r.id AS role_id, r.title AS role_title, d.name AS department_name
  //     FROM employee e
  ...
  //   console.log("Result from database:", result);
  //   const employees = result.rows; // Ensure employees is declared and initialized here
  ...
  //   // Assuming Tables.viewEmployeesByDepartmentTable is properly handling the input
...
const updateDepartmentName = async (departmentId, newDepartmentName) => {
  ...
  try {
    ...
    return res.rows[0]; // Returns the updated department record.
  } catch (error) {
...
const deleteRole = async (roleId) => {
  try {
    ...
    if (employees.rows.length > 0) {
      ...
      // Return the list of employees for further handling
...
//     try {
//       const roles = await this.queries.viewAllRoles(); // Ensure this is awaited properly
//       if (!roles || roles.length === 0) {
//         console.log("No roles to display.");
//         return;
//       }
//       const table = Tables.viewAllRolesTable(roles);
